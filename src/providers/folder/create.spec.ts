...
it('should create an item in the database', async () => {
    ...
    const item = filesystem.readSync.json('/db/01/index.json')
...
it('should create an item with an id in the database', async () => {
    ...
    expect(filesystem.existsSync('/db/random-id')).toBe(true)

    expect(filesystem.readSync.json('/db/random-id/index.json')).to.containSubset({
        title: 'Hello',
    })
})
