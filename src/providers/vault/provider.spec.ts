...
const { makeItem, makeManyItems, makeEncryption } = createFactories({
    filesystem,
    parser,
    root: config.path,
})

let provider: ReturnType<typeof mount>
...
beforeEach(() => {
    filesystem.removeSync(config.path)

    filesystem.mkdirSync(config.path)
