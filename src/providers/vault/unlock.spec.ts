...
it('should unlock an item in the vault', async () => {
    ...
    expect(result).toEqual(
        expect.arrayContaining([expect.objectContaining({ name: 'index.json', encrypted: false })])
    )
...
it('should unlock an item when it is encrypted by lock function', async () => {
    filesystem.writeSync.json(`${root}/item1/index.json`, { name: 'Item 1' }, { recursive: true })
